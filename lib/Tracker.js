...
Tracker = function(proto, conn, opts) {
  ...
  this.connection.on('data', function( buffer ) {
    ...
    var messages = buffer.toString('utf8').split( tracker.Message.EOL );
    
    for (var key in messages) {
      var message = tracker.Message.parseMessage( messages[key] );

      if (!message) {
        tracker.emit('error', new Error('Error parsing message'), messages[key]);
        return;
      }
      
      tracker.emit('packet.in', message.raw);
